...
app.layout = html.Div([
    dcc.Tabs(id="tabs", children=[
        dcc.Tab(label='Country and Goals During Tournament', children=[
            html.Div([
                dcc.Graph(
                    id='countrygoals',
                     figure={
...
'layout': {'title': 'Average Goals By Venue'}}
),
])
]),
...
# df2 = pd.DataFrame(all_games, columns=['attendance', 'away_team_country','home_team_country', 'location','status', 'time', 'venue', 'weather', 'winner'])
# Parsed_data = json.loads(all_games)


# def stats_table(): #new
#     total_country= [x[1] for x in return_country_and_its_total_goals()]
#     avg_goals= [x[1] for x in return_average_goals_per_game()]
#     ot= [x[1] for x in return_average_ot_per_game()]
#     teamdc= [x[1] for x in return_average_dc_per_game_desc()]
#     teamddc2 =[x[1] for x in return_dc_per_team_desc()]
#     avgpa=[x[1] for x in return_average_pa_per_game()]
#     avgdc=[x[1] for x in return_average_dc_per_game_desc()]
#     country= [x[0] for x in return_average_goals_per_game()]
    # return[country,total_country,avg_goals,ot,teamdc,teamddc2,avgpa,avgdc]
...
#
#
#   html.H1(children='Country and Average Goals During Tournament!'),
#   dcc.Graph(
#     id='countrygoals',
#     figure={
#       'data': [{'x': average_goals()[0], 'y': average_goals()[1], 'type': 'bar'}],
